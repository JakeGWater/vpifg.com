...
def download_images(app, env):
    ...
    for src in (app.builder.status_iterator
        ...
        # Strip out leading / or path.join hates us
        local_cache = env.remote_images[src]
        if local_cache[0] == '/':
            local_cache = local_cache[1:]
        dst = os.path.join(env.srcdir, local_cache)
...
def configure_backend(app):
    ...
    cache_dir = os.path.join(app.env.srcdir, config['cache_path'])
    ensuredir(cache_dir)
